    ...
    <div class="row body">
      <div class="bs-example" data-example-id="striped-table">
         ...
         <table class="table table-striped">
            ...
            <tbody>
            <?php 
              $query = mysqli_query($conn, "SELECT * FROM  `webcasts` ORDER BY `datetime` DESC");
            ...
            ?>
               <tr>
                  ...
                  <?php 
                    if($row["visibility"] == 0){$class = "btn-info"; }else{ $class = "btn-primary"; }
                    if($row["visibility"] == 0){$text = "Hide"; }else{ $text = "Show"; }
                  ?>
                  <td><button class="btn <?php echo $class; ?> data-hide" data="<?php echo $row["id"]; ?>"><?php echo $text; ?></button></td>
                  <td>
...
$(".data-hide").click(function(){
    ...
    var v = 1;
    var command = currentitem.text();
    command == "Show" ? v = 0 : v = 1;
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            if(this.responseText == "success"){
              if(command == "Hide"){
                currentitem.attr("id", "show");
                currentitem.attr("class", "btn btn-primary data-hide");
                currentitem.text("Show");
              }
              else{
                currentitem.attr("id", "hide");
                currentitem.attr("class", "btn btn-info data-hide");
                currentitem.text("Hide");
              }
            }

        }
    };
    xhttp.open("GET", "updatestatus.php?id="+ $(this).attr("data") +"&v=" + v, true);
    xhttp.send();
});
